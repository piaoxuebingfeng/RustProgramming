...
- 3、当所有者(变量)离开作用范围时，这个值将被丢弃(drop).
...
```
  ...
  s 从创建开始就有效，然后有效期持续到它离开作用域为止。

  就作用域来说，Rust 语言跟其他编程语言没什么区别。
...
​	代码首先将5绑定到变量x,接着拷贝x的值赋给y，最终x和y都等于5.

​    因为整数是 Rust 基本数据类型，是固定大小的简单值，因此这两个值都是通过**自动拷贝**的方式来赋值的，都被存在栈中，完全无需在堆上分配内存。
...
```bash
$ rustc ownershiptest2.rs 
error[E0382]: borrow of moved value: `s1`
 --> ownershiptest2.rs:5:28
  |
...
## 引用与借用
  ...
  上文中提到，仅仅支持通过转移所有权的方式获取一个值，那样会让程序设计变得复杂。Rust 能否像其它编程语言一样，使用某个变量的指针或引用呢？答案是肯定的。

  Rust 通过 `借用(Borrowing)` 这个概念来达成上述目的。获取变量的引用，称之为借用(borrowing)。

 如现实生活中，如果一个人拥有某样东西，你可以从他那里借来，当使用完毕后，也必须要物归原主。

  

### 引用与解引用

  规引用是一个指针类型，指向了对象存储的内存地址。在下面代码中，我们创建一个 `i32` 值的引用 `y`，然后使用解引用运算符来解出 `y` 所使用的值。
...
help: consider dereferencing here
 --> /home/qiao/.rustup/toolchains/stable-x86_64-unknown-linux-gnu/lib/rustlib/src/rust/library/core/src/macros/mod.rs:46:35
  |
...
```bash
$ rustc borrowingtest2.rs 
error[E0596]: cannot borrow `*some_string` as mutable, as it is behind a `&` reference
 --> borrowingtest2.rs:8:5
  |
...
```bash
$ rustc  borrowingtest3.rs 
error[E0499]: cannot borrow `s` as mutable more than once at a time
 --> borrowingtest3.rs:6:14
  |
5 |     let r1 = &mut s;
...
```bash
$ rustc  borrowingtest4.rs 
error[E0502]: cannot borrow `s` as mutable because it is also borrowed as immutable
 --> borrowingtest4.rs:7:14
  |
5 |     let r1 = &s; // 没问题
...
虽然这种借用错误有的时候会让我们很郁闷，但是你只要想想这是 Rust 提前帮你发现了潜在的 BUG，其实就开心了，虽然减慢了开发速度，但是从长期来看，大幅减少了后续开发和运维成本。](https://course.rs/basic/ownership/borrowing.html#悬垂引用dangling-references)
